...
public:
	...
	static std::string encode( const std::string &value ) noexcept {
		...
		for ( auto &chr : value ) {
			if ( !( ( chr >= '0' && chr <= '9' ) || ( chr >= 'A' && chr <= 'Z' ) || ( chr >= 'a' && chr <= 'z' ) || chr == '-' || chr == '.' || chr == '_'
					|| chr == '~' ) ) {
...
public:
	 ...
	 */
	static bool parse( std::istream &stream, std::string &method, std::string &path, std::string &query_string, std::string &version,
